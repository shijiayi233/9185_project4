...
glmm_sae4 <- glmmTMB(sae ~ group * time + sex + age + (1|site:id), data = d, family = binomial())
#gee
gee_sae <- geeglm(
...
library(emmeans)
emmeans(model_firth, pairwise ~ group | time)

# Extract linear predictor (logit)
d$pred <- predict(glmm_sae4, type = "link")

# Linearity plot: age vs. logitggplot(d, aes(x = age, y = pred)) +
  geom_point(alpha = 0.3) +
  labs(title = "Linearity Check: Age vs. Logit", 
       y = "Predicted Logit", x = "Age")

re_list <- ranef(glmm_sae4)$cond[["site:id"]]
re_intercepts <- re_list[["(Intercept)"]]
par(mfrow = c(1, 2))
hist(re_intercepts,
     main = "Histogram of Random Intercepts (site:id)",
     xlab = "Random Intercept Value")
qqnorm(re_intercepts,
       main = "QQ Plot of Random Intercepts (site:id)")
